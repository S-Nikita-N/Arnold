...
class BodyGroup:
    """
    Группа наблюдений, объединённых по телу/мышце для body-level
    tokenization.
    """
...
class ObservationParser:
    ...
    def from_env(cls, env, history_len: int = 5) -> "ObservationParser":
        """
        Создаёт парсер из среды Kinesis/MyoHumanIm.

        Args:
            env: Среда MyoHumanIm или аналог
            history_len: Длина истории
        """
        cfg = env.cfg
        ...
        if not hasattr(env, "tracked_bodies"):
            raise AttributeError(
                f"env {type(env).__name__} has no 'tracked_bodies' "
                f"attribute. Available attrs: "
                f"{
                    [
                        a
                        for a in dir(env)
    ...
    def get_body_groups(self, granularity: str = "per_body") -> list[BodyGroup]:
        """
        Строит группы наблюдений с настраиваемой гранулярностью.

        Granularity (от самого мелкого к самому крупному):
          "scalar"   — каждый скаляр = свой токен (~1583).
                       Эквивалент SensoryEncoder.
