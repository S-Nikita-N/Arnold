...
SIDE_TOKENS: list[str] = ["r", "l", "c"]

COORD_TOKENS: list[str] = ["x", "y", "z", "qw", "qx", "qy", "qz"]
...
BODY_TOKENS: list[str] = [
    ...
    # Нельзя удалить из XML — обученная модель зависит от его
    # наблюдений (453 -> 438 без него).
    "head_attach",
]
...
class SensorimotorVocabulary(nn.Module):
    ...
    def _apply(self, fn):
        """Переносит _idx_cache при .to()/.cpu()/device-переносе."""
        super()._apply(fn)
    ...
    # Методы nn.Module (parameters, state_dict, load_state_dict, to,
    # train, eval) наследуются автоматически и работают корректно
    # с self.embeddings
