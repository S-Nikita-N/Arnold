...
from myohuman.utils import np_transform_utils as npt_utils  # noqa: E402
...
class KinesisTeacher(nn.Module):
    ...
    def forward_batch(
        ...
        Returns:
            actions_mh: (B, 338) actions in MyoHuman muscle order.
                Arm indices are 0.
...
def eval_worker(
    ...
    """Env worker: builds obs, steps env, reports per-frame
    metrics + trajectory data."""
...
def run_eval(args):
    ...
    for r in per_motion:
        ...
        obs_arr = traj[
            "obs"
        ]  # (T+1, obs_dim) — T+1 because includes initial obs
