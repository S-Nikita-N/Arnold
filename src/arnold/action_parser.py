...
Стратегии грануляции:
  ...
  "hybrid"      — анатомические фасции для торса + функциональные
                  группы для конечностей.
"""
...
# regex фасций + словари конечностей для стратегий грануляции
...
ANATOMICAL_TORSO_REGEX_MAP: list[tuple[str, str]] = [
    (
        r"^PECM[123]_",
        "pectoralis",
    ),  # фасции большой грудной — большая грудная мышца
    ...
    (
        r"^glmax[123]_",
        "gluteus_max",
    ),  # фасции большой ягодичной — большая ягодичная мышца
    (
        r"^glmed[123]_",
        "gluteus_med",
    ),  # фасции средней ягодичной — средняя ягодичная мышца
    (
        r"^glmin[123]_",
        "gluteus_min",
    ),  # фасции малой ягодичной — малая ягодичная мышца
    (r"^IL_", "iliocostalis"),  # фасции IL_* — подвздошно-рёберная мышца
    (
        r"^LTpT_",
        "longissimus",
    ),  # фасции LTpT_* (грудной ход) — длиннейшая мышца спины
    (
        r"^LTpL_",
        "longissimus",
    ),  # фасции LTpL_* (поясничный ход) — длиннейшая мышца спины
    (r"^MF_", "multifidus"),  # фасции MF_* — многораздельная мышца
    (r"^QL_", "quadratus_lumborum"),  # фасции QL_* — квадратная мышца поясницы
    (
        r"^EO[1-6]_",
        "external_oblique",
    ),  # фасции EO_* — наружная косая мышца живота
    (
        r"^IO[1-6]_",
        "internal_oblique",
    ),  # фасции IO_* — внутренняя косая мышца живота
    ...
    (
        r"^ercspn_",
        "erector_spinae_cervical",
    ),  # ercspn — шейный разгибатель, упрощённая мышца в модели
    (
        r"^intobl_",
        "internal_oblique_simple",
    ),  # intobl — внутренняя косая (упрощ.), одна мышца в модели
    (
        r"^extobl_",
        "external_oblique_simple",
    ),  # extobl — наружная косая (упрощ.), одна мышца в модели
]
...
FUNCTIONAL_TORSO_REGEX_MAP: list[tuple[str, str]] = [
    (
        r"^EO[1-6]_",
        "spine_flexors",
    ),  # наружная косая живота, фасции — сгибатели позвоночника (корпус)
    (
        r"^IO[1-6]_",
        "spine_flexors",
    ),  # внутренняя косая живота, фасции — сгибатели позвоночника (корпус)
    (
        r"^rect_abd_",
        "spine_flexors",
    ),  # прямая мышца живота — сгибатели позвоночника (корпус)
    (
        r"^intobl_",
        "spine_flexors",
    ),  # внутр. косая упрощ. — сгибатели позвоночника (корпус)
    (
        r"^extobl_",
        "spine_flexors",
    ),  # наруж. косая упрощ. — сгибатели позвоночника (корпус)
    (
        r"^IL_",
        "spine_extensors",
    ),  # подвздошно-рёберная, фасции — разгибатели позвоночника (корпус)
    (
        r"^LTpT_",
        "spine_extensors",
    ),  # длиннейшая грудной ход — разгибатели позвоночника (корпус)
    (
        r"^LTpL_",
        "spine_extensors",
    ),  # длиннейшая поясничный ход — разгибатели позвоночника (корпус)
    (
        r"^MF_",
        "spine_extensors",
    ),  # многораздельная — разгибатели позвоночника (корпус)
    (
        r"^QL_",
        "spine_extensors",
    ),  # квадратная поясничная — разгибатели позвоночника (корпус)
    (
        r"^Ps_",
        "spine_extensors",
    ),  # параспинальные — разгибатели позвоночника (корпус)
    (
        r"^ercspn_",
        "spine_extensors",
    ),  # шейный столб разгибателей — разгибатели позвоночника (корпус)
    (
        r"^PECM[123]_",
        "shoulder_flexors",
    ),  # большая грудная, фасции — сгибатели плечевого сустава
    (
        r"^DELT1_",
        "shoulder_flexors",
    ),  # дельтовидная передняя фасция — сгибатели плечевого сустава
    (
        r"^LAT[123]_",
        "shoulder_extensors",
    ),  # широчайшая, фасции — разгибатели плечевого сустава
    (
        r"^DELT3_",
        "shoulder_extensors",
    ),  # дельтовидная задняя фасция — разгибатели плечевого сустава
    (
        r"^DELT2_",
        "shoulder_abductors",
    ),  # дельтовидная средняя фасция — отводящие мышцы плеча
    (
        r"^glmax[123]_",
        "hip_extensors",
    ),  # большая ягодичная, фасции — разгибатели бедра в тазобедренном суставе
    (
        r"^glmed[123]_",
        "hip_abductors",
    ),  # средняя ягодичная, фасции — отводящие мышцы бедра
    (
        r"^glmin[123]_",
        "hip_abductors",
    ),  # малая ягодичная, фасции — отводящие мышцы бедра
]

# --- Hybrid: функц. группы конечностей (торс: ANATOMICAL_TORSO_REGEX_MAP) ---
HYBRID_LIMB_MAP: dict[str, str] = {
    "addbrev": "hip_adductors",  # короткая приводящая бедра
    "addlong": "hip_adductors",  # длинная приводящая бедра
    "addmagDist": "hip_adductors",  # приводящая большая дистальная часть
    "addmagIsch": "hip_adductors",  # приводящая большая седалищная часть
    "addmagMid": "hip_adductors",  # приводящая большая средняя часть
    "addmagProx": "hip_adductors",  # приводящая большая проксимальная часть
    "bflh": "knee_flexors",  # двуглавая бедра длинная головка
    "bfsh": "knee_flexors",  # двуглавая бедра короткая головка
    ...
    "gaslat": "ankle_plantarflexors",  # икроножная латеральная
    "gasmed": "ankle_plantarflexors",  # икроножная медиальная
    "soleus": "ankle_plantarflexors",  # камбаловидная
    "tibpost": "ankle_plantarflexors",  # задняя большеберцовая
    "perlong": "ankle_plantarflexors",  # длинная малоберцовая
    "perbrev": "ankle_plantarflexors",  # короткая малоберцовая
    "tibant": "ankle_dorsiflexors",  # передняя большеберцовая
    "edl": "ankle_dorsiflexors",  # длинный разгибатель пальцев стопы
    "ehl": "ankle_dorsiflexors",  # длинный разгибатель большого пальца стопы
    "fdl": "ankle_plantarflexors",  # длинный сгибатель пальцев стопы
    "fhl": "ankle_plantarflexors",  # длинный сгибатель большого пальца стопы
    "psoas": "hip_flexors",  # поясничный мышечный столб
    "iliacus": "hip_flexors",  # подвздошная
    "recfem": "hip_flexors",  # прямая мышца бедра
    "sart": "hip_flexors",  # портняжная
    "vaslat": "knee_extensors",  # латеральная широкая мышца бедра
    "vasmed": "knee_extensors",  # медиальная широкая мышца бедра
    "vasint": "knee_extensors",  # промежуточная широкая мышца бедра
    "tfl": "hip_abductors",  # широкая фасция бедра / тракт
    "piri": "hip_abductors",  # грушевидная — отводящие мышцы бедра
    "grac": "hip_adductors",  # тонкая мышца — приводящие мышцы бедра
    "CORB": "shoulder_flexors",  # подключично-лопаточная
    "TMAJ": "shoulder_extensors",  # большая круглая
    "TRIlong": "elbow_extensors",  # трёхглавая плеча длинная головка
    ...
    "BIClong": "elbow_flexors",  # двуглавая плеча длинная головка
    "BICshort": "elbow_flexors",  # двуглавая плеча короткая головка
    "BRA": "elbow_flexors",  # плечевая — сгибатели локтевого сустава
    "BRD": "elbow_flexors",  # лучевая — сгибатели локтевого сустава
    "TRIlat": "elbow_extensors",  # трёхглавая латеральная головка
    "TRImed": "elbow_extensors",  # трёхглавая медиальная головка
    "ANC": "elbow_extensors",  # локтевая — разгибатели локтевого сустава
    "SUP": "pronators_supinators",  # круглый пронатор
}
...
FUNCTIONAL_LIMB_MAP: dict[str, str] = {
    ...
    "recfem": "hip_flexors",  # прямая мышца бедра
    "sart": "hip_flexors",  # портняжная
    "bflh": "hip_extensors",  # двуглавая длинная головка
    "semimem": "hip_extensors",  # полуперепончатая
    "semiten": "hip_extensors",  # полусухожильная
    ...
    "tfl": "hip_abductors",  # широкая фасция бедра / тракт
    "piri": "hip_abductors",  # грушевидная — отводящие мышцы бедра
    "vaslat": "knee_extensors",  # латеральная широкая бедра
    "vasmed": "knee_extensors",  # медиальная широкая бедра
    "vasint": "knee_extensors",  # промежуточная широкая бедра
    "bfsh": "knee_flexors",  # двуглавая короткая головка
    "gaslat": "knee_flexors",  # икроножная латеральная
    "gasmed": "knee_flexors",  # икроножная медиальная
    "soleus": "ankle_plantarflexors",  # камбаловидная
    "tibpost": "ankle_plantarflexors",  # задняя большеберцовая
    "perlong": "ankle_plantarflexors",  # длинная малоберцовая
    "perbrev": "ankle_plantarflexors",  # короткая малоберцовая
    ...
    "fhl": "ankle_plantarflexors",  # длинный сгибатель большого пальца стопы
    "tibant": "ankle_dorsiflexors",  # передняя большеберцовая
    "edl": "ankle_dorsiflexors",  # длинный разгибатель пальцев
    "ehl": "ankle_dorsiflexors",  # длинный разгибатель большого пальца стопы
    "CORB": "shoulder_flexors",  # подключично-лопаточная
    "TMAJ": "shoulder_extensors",  # большая круглая
    "SUPSP": "shoulder_abductors",  # надостная — отводящие мышцы плеча
    ...
    "SUBSC": "rotator_cuff",  # подлопаточная — манжета вращателей плеча
    "TMIN": "rotator_cuff",  # малая круглая — манжета вращателей плеча
    "BIClong": "elbow_flexors",  # двуглавая длинная головка
    "BICshort": "elbow_flexors",  # двуглавая короткая головка
    "BRA": "elbow_flexors",  # плечевая — сгибатели локтевого сустава
    "BRD": "elbow_flexors",  # лучевая — сгибатели локтевого сустава
    "TRIlong": "elbow_extensors",  # трёхглавая длинная головка
    "TRIlat": "elbow_extensors",  # трёхглавая латеральная головка
    "TRImed": "elbow_extensors",  # трёхглавая медиальная головка
    "ANC": "elbow_extensors",  # локтевая — разгибатели локтевого сустава
    "SUP": "pronators_supinators",  # круглый пронатор
...
# жёсткий маппинг granule → muscle bases
